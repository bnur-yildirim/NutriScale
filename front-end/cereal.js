document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('recipes.json')
        ...
        .then(data => {
            const recipe = data.recipes.find(recipe => recipe.name === "Cereal");
            ...
            // Populate the ingredient dropdown, excluding 30g and 240g ingredients
            const ingredientSelect = document.getElementById('ingredient-select');
            recipe.ingredients
                .filter(ingredient => !ingredient.includes('30g') && !ingredient.includes('240g')) // Exclude 30g and 240g ingredients
                .forEach(ingredient => {
                    const option = document.createElement('option');
                    option.value = ingredient.split(" ")[0];  // Assuming ingredient format like 'milk 100g'
                    option.textContent = ingredient.split(" ")[0];  // Display ingredient name
                    ingredientSelect.appendChild(option);
                });

            // Store the original recipe for adjustment
            window.originalRecipe = recipe;
        })
