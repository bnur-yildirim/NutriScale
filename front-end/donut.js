document.addEventListener('DOMContentLoaded', () => {
    // Fetch the recipe data from JSON (adjusted to match the donut recipe structure)
    fetch('recipes.json')  // Use the correct path if necessary
        .then(response => response.json())
        .then(data => {
            const donutRecipe = data.recipes.find(recipe => recipe.name === "Donuts");

            // Populate the recipe name
            document.getElementById('recipe-name').textContent = donutRecipe.name;

            // Populate the ingredients
            const ingredientsList = document.getElementById('ingredients-list');
            donutRecipe.ingredients.forEach(ingredient => {
            ...
            donutRecipe.steps.forEach(step => {
                const listItem = document.createElement('li');
            ...
            window.originalRecipe = donutRecipe;
        })
